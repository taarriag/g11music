...
\begin{enumerate}
	\item Cuando el timer TMR1 está en enable, la pantalla LCD se paraliza. Vemos que una causa probable de este error es que la interrupción de timer detiene el flujo normal del programa ejecutando la subrutina de la interrupción. Ahora, si es que se estaba ejecutando una instrucción de la pantalla LCD mientras ocurre una interrupción, puede causar un error en el LCD. Para evitar esto, mientra se ejecutan las instrucciones de la pantalla, se desactivan todas las instrucciones haciendo clear del registro INTCON$<$7$>$. A pesar de lo anterior, de todas formas falla el funcionamiento del LCD cuando se activa el timer.
...
\begin{itemize}
	\item Para establecer el tiempo en el que el dispositivo detendrá su reproducción se utiliza únicamente un botón, existiendo una flexibilidad reducida para cambiar el tiempo (disminuirlo por ejemplo)
	\item Existen solamente 9 estados posibles para distintos tipos de música según los 3 niveles de sonido y de luz respectivamente.
