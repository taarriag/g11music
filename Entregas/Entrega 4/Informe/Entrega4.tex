...
\section{Implementación del dispositivo}
...
\subsection{Implementación del \textit{hardware}}
...
\subsubsection{SmartMusic2Winamp}
Este software ha sido desarrollado en C\verb@#@ y permite comunicar el PC con el PIC desde el lado del PC. Su funcionalidad es recibir datos desde el PIC a través del puerto serial
y en función éstos realizar una acción particular en Winamp\footnote{Software de reproducción musical. Más información: www.winamp.com}: Cambiar la lista de reproducción, iniciar/detener la reproducción, avanzar a la siguiente pista, retroceder a la pista anterior. Además se encarga de enviar información hacia el PIC, en particular el título de la canción que se está reproduciendo actualmente.

El software recibe bytes desde el PIC, los cuales corresponden a caracteres ASCII en función de los cuales el software decide la acción a ejecutar. Para esto utiliza un sistema de eventos, en el cual los métodos que determinan la acción a ejecutar se encuentran suscritos a la llegada de nuevos datos desde el PIC. Por otra parte, existe un método suscrito al cambio de una canción en Winamp que envía el nombre de la nueva canción hacia el PIC.

Para utilizar SmartMusic2Winamp, se deben crear listas de reproducción de extensión .m3u con la sintaxis [NivelLuz][NivelAudio].m3u, donde NivelLuz y NivelAudio pueden tomar los valores 1,2 o 3 dependiendo del nivel sensado correspondiente a esa lista. Estas listas deben ser guardadas en la carpeta de SmartMusic2Winamp. Por lo tanto, esta carpeta debería contener los archivos 11.m3u,12.m3u,13.m3u,21.m3u,22.m3u,23.m3u,31.m3u,32.m3u,33.m3u.

\subsubsection{Software Assembler}
Este software se encuentra almacenado en el PIC. Sus funciones son:

\begin{itemize}
	\item Determinar el nivel de luz y volumen en función de los valores detectados en los sensores.
	\item Configurar y escribir en la pantalla LCD información acerca de la pista de reproducción y los niveles detectados.
	\item Comunicarse con el PC a través del puerto serial, enviando información sobre los sensores y recibiendo información acerca de la canción reproducida.
	\item Determinar y manejar interrupciones de los botones, para determinar la acción a enviar al computador (\textit{play/pause,backward,forward})
	\item Determinar y manejar interrupciones de timer, a fin de apagar el dispositivo transcurrido cierto tiempo (determinado por el usuario)
\end{itemize}

El software comienza configurando los registros del PIC en función de las funcionalidades requeridas (el detalle de estta configuración se encuentra en la sección Implementación de Hardware). Luego entra en un ciclo dentro del cual se realizan las siguientes acciones:
\begin{enumerate}
	\item Obtiene voltaje correspondiente a la cantidad de luz y volumen del ambiente.
	\item Determina el nivel correspondiente a los valores detectados.
	\item Envía información al PC sobre los niveles detectados.
	\item Recibe información desde el PC con el título de la canción.
	\item Escribe en la pantalla LCD el título de la canción y los niveles de luz y sonido.
\end{enumerate}

Y además debe responder a las siguientes interrupciones:
\begin{itemize}
	\item Al pulsar un boton entra a una interrupción en la cual envía la acción correspondiente al PC.
	\item Al ocurrir una interrupción de \textit{Timer1 Overflow} decrementa el contador de tiempo que le indica cuando apagarse.
\end{itemize}
...
\begin{itemize}
	...
	\item El segundo problema que surgió se relaciona con la configuración del \textit{timer} TMR1. Con respecto a este módulo, surgieron dos problemas que no logramos resolver:
	
\begin{enumerate}
	\item Cuando el \textit{timer} TMR1 está en \textit{enable}, la pantalla LCD se paraliza. Vemos que una causa probable de este error es que la interrupción de \textit{timer} detiene el flujo normal del programa ejecutando la subrutina de la interrupción. Ahora, si es que se estaba ejecutando una instrucción de la pantalla LCD mientras ocurre una interrupción, puede causar un error en el LCD. Para evitar esto, mientras se ejecutan las instrucciones de la pantalla, se desactivan todas las instrucciones haciendo \textit{clear} del registro INTCON$<$7$>$. A pesar de lo anterior, de todas formas falla el funcionamiento del LCD cuando se activa el \textit{timer}.
	\item Se observó al simular el código que al utilizar el \textit{timer} TMR1, cada vez que transcurrió cierto tiempo, se interrumpe la ejecución actual del programa comenzando nuevamente su ejecución. Creemos que el problema se podría deber al \textit{Watchdog Timer}, el cual podría estar reiniciando el programa cuando este está en un \textit{loop} muy largo. Por este motivo desactivamos este \textit{timer}, pero no obtuvimos resultados favorables.  
\end{enumerate}

\item Un tercer problema encontrado se relaciona con ciertas fallas del \textit{hardware}. En primer lugar, vimos que los puertos A0 y D4 no funcionan correctamente. Para solucionarlo simplemente no los utilizamos. Además observamos que el sensor de sonido falló y dejó de medir el sonido del ambiente. Para solucionar esto y probar el funcionamiento de nuestro dispositivo decidimos utilizar un potenciómetro en reemplazo del sensor con la idea de reemplazarlo por un sensor funcional en un futuro cercano. Como el dispositivo funciona con el potenciómetro, vemos que la falla es efectivamente del sensor.
\end{itemize}
...
\begin{itemize}
\item Como fue mencionado en uno de los problemas anteriores, en nuestro dispositivo no funciona correctamente el \textit{timer} que sería utilizado para detener la reproducción.
